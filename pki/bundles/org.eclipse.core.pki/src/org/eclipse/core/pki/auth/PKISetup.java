...
public class PKISetup implements BundleActivator, IStartup {
		...
		if (IncomingSystemProperty.SETTINGS.checkType()) {
			if (IncomingSystemProperty.SETTINGS.checkKeyStore(pin)) {
				if (PKIState.CONTROL.isPKCS12on()) {
					processPKCS12Context();
					/*
					 * try {
					 * 
					 * keystoreContainer = Optional.ofNullable(
					 * KeyStoreManager.INSTANCE.getKeyStore(System.getProperty(
					 * "javax.net.ssl.keyStore"), //$NON-NLS-1$
					 * System.getProperty("javax.net.ssl.keyStorePassword"), //$NON-NLS-1$
					 * KeyStoreFormat.valueOf(System.getProperty("javax.net.ssl.keyStoreType"))));
					 * //$NON-NLS-1$
					 * 
					 * if ((keystoreContainer.isEmpty()) ||
					 * (!(KeyStoreManager.INSTANCE.isKeyStoreInitialized()))) {
					 * LogUtil.logError("PKISetup - Failed to Load a Keystore.", null);
					 * //$NON-NLS-1$ PKIState.CONTROL.setPKCS12on(false);
					 ...
					 * SecurityFileSnapshot.INSTANCE.restoreProperties(); } else {
					 * LogUtil.logError("A Keystore and Password are detected.", null);
					 * //$NON-NLS-1$ keyStore = keystoreContainer.get(); setKeyStoreLoaded(true); }
					 * } catch (Exception e) { LogUtil.logError("Failed to Load Keystore.", e);
					 * //$NON-NLS-1$ }
					 */
				}
	...
	public void processPKCS12Context() {
		try {
